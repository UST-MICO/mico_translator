...
class TestTranslatorCustom(TranslatorTest):
    ...
    def test_apply_script_1(self):
        """
        Tests if the AbstractTranslatorUserDefinedFunctions is able to load a user defined function and apply it on a message
        """
        ...
        trns = TranslatorCUSTOM().init(user_functions=config['function_scripts'])
    ...
    def test_import_dependencies(self):
        ...
        trns = TranslatorCUSTOM().init(user_functions=config['function_scripts'], dependencies=config['dependencies'])
        result = trns.translate(cloud_event)
